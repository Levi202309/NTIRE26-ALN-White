...
def dwt_init(x):
    _, _, h, w = x.shape
    # Pad the height and width if they are odd
    if h % 2 == 1 or w % 2 == 1:
    ...
    # Crop back to match expected output size
    new_h, new_w = h // 2, w // 2  # Target size for downsampled output
    x_LL = x_LL[:, :, :new_h, :new_w]
    x_HL = x_HL[:, :, :new_h, :new_w]
    x_LH = x_LH[:, :, :new_h, :new_w]
    x_HH = x_HH[:, :, :new_h, :new_w]
...
class DWT_transform(nn.Module):
    ...
    def forward(self, x):
        ...
        return dwt_low_frequency,dwt_high_frequency # here, h/w possibly become (h+1)/2
...
class dwt_ffc_UNet2(nn.Module):
    ...
    def forward(self, x):
        ...
        if h % 8 != 0 or w % 8 != 0:
            w_padwidth, h_padwidth = 8 - w % 8, 8 - h % 8
            x = F.pad(x, (0, w_padwidth, 0, h_padwidth), mode='reflect')
        
        conv_start=self.initial_conv(x)
        conv_start=self.bn1(conv_start)

        conv_out1 = self.layer1(conv_start) 
        dwt_low_0, dwt_high_0=self.DWT_down_0(x) # h or w won't be floored as x is padded
        out1=torch.cat([conv_out1, dwt_low_0], 1)
        #_, _, h_out1, w_out1 = out1.shape

        #if h_out1 % 2 == 1 or w_out1 % 2 == 1:
        #    out1 = F.pad(out1, (0, w_out1 % 2, 0, h_out1 % 2), mode='reflect')

        conv_out2 = self.layer2(out1)

        dwt_low_1, dwt_high_1= self.DWT_down_1(out1) 
        out2 = torch.cat([conv_out2, dwt_low_1], 1)
        #_, _, h_out2, w_out2 = out2.shape

        #if h_out2 % 2 == 1 or w_out2 % 2 == 1:
        #    out2 = F.pad(out2, (0, w_out2 % 2, 0, h_out2 % 2), mode='reflect')
        ...
        out3_ffc= self.FFCResNet(out3) # bs, 64, 93, 125


        dout3 = self.dlayer6(out3_ffc) # bs, 64, 186, 250
        ...
        #print(out3.shape, out3_ffc.shape, dout3.shape, out2.shape, dwt_high_1.shape)
...
class knowledge_adaptation_convnext(nn.Module):
    ...
    def forward(self, input):
        ...
        if h % 16 != 0 or w % 16 != 0:
            w_padwidth, h_padwidth = 16 - w % 16, 16 - h % 16
            input = F.pad(input, (0, w_padwidth, 0, h_padwidth), mode='reflect')
        x_layer1, x_layer2, x_output = self.encoder(input)

        x_mid = self.attention0(x_output)  #[1024,24,24], 46, 62

        x = self.up_block(x_mid)      #[256,48,48] 92, 124
        x = self.attention1(x)

        #print(x_layer1.shape, x_layer2.shape, x_output.shape, x_mid.shape)

        x = torch.cat((x, x_layer2), 1)  #[768,48,48] 92x124 93x125 
