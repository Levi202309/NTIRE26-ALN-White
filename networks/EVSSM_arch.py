...
class EVS(nn.Module):
    ...
    def forward(self, x):
        if self.att:
            ...
            # 恢复翻转 (非常重要！否则图像空间结构会乱)
            # 原代码漏写了恢复翻转的逻辑，如果不加 grids_inverse，必须手动恢复！
            # --- 【新增逻辑：必须逆转回去】 ---
            if self.idx % 2 == 0:
                 x = torch.transpose(x, dim0=-2, dim1=-1).contiguous()
            if self.idx % 2 == 1:
...
if __name__ == "__main__":
    ...
    # 加载你的 EVSSM 模型
    model = EVSSM(inp_channels=3, out_channels=3, dim=48, num_blocks=[6, 6, 12], ffn_expansion_factor=3, bias=False)
    model.to(device)  # 将模型移到指定设备
    
    # 输入图像的形状（注意这里传递的是一个形状元组）
    input_shape = (3, 600, 960)  # Batch size = 1, Channels = 3, Height = 1200, Width = 1920
    
    # 使用 ptflops 来计算模型的 MACs 和参数量
    macs, params = get_model_complexity_info(model, input_shape, verbose=False, print_per_layer_stat=False)
    
    # 检查 macs 和 params 是否为 None
    if macs is None or params is None:
        print("无法计算模型的 MACs 或参数量。请检查模型或输入的配置。")
    else:
        # 输出模型的 MACs 和参数量
        print(f"MACs: {macs / 1e9:.2f} GFLOPs")  # 转换为 GFLOPs（十亿次浮点运算）
