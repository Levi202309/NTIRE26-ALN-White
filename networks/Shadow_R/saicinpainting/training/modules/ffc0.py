...
class SpectralTransform(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.enable_lfu:
            ...
            # Check if height/width is odd
            pad_h = h % split_no  # 1 if odd, 0 if even
            pad_w = w % split_no  # 1 if odd, 0 if even

            # Pad x to make it divisible by split_no
            x = F.pad(x, (0, pad_w, 0, pad_h), mode="reflect")

            # Recalculate new height/width after padding
            ...
            # Crop back to original size if we added padding
            xs = xs[:, :, :h - pad_h, :w - pad_w]
            x = x[:, :, :h - pad_h, :w - pad_w]
        else:
            xs = 0
